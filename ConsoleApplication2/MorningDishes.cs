...
{
    public class MorningDishes : IDish
    {
        /// <summary>
        ...
        public string GetOutput(string[] strArray)
        {
            ...
            if (objDishList.Where(d=>d.id==3).FirstOrDefault()!=null && objDishList.Where(d=>d.id==3).FirstOrDefault().count > 1)
            {
                finalString = finalString.Replace("coffee", string.Format("coffee(x{0})", objDishList.Where(d => d.id == 3).FirstOrDefault().count.ToString()));
            }
        ...
        /// <summary>
        /// Method is used to get the Dish Name and if Dish Type is 3 (drink), then allow multiple orders.
        /// </summary>
        ...
        private string GetDishesName(string timeOfDay, string dishType, List<DishDO> objDishDOList)
        {
            ...
            if (dishType == "3" && (objDishDOList.Where(i => i.id == 3).FirstOrDefault() != null && objDishDOList.Where(i => i.id == 3).FirstOrDefault().count > 0))
            {
                objDishDOList.Where(i => i.id == 3).FirstOrDefault().count = objDishDOList.Where(i => i.id == 3).FirstOrDefault().count + 1;
                return "";
            }
